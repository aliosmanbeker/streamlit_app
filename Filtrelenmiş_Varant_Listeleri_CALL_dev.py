...
def show_page():
    ...
    st.write("Arkhon için gerekli CALL warrant ve varantsız spot verilerini gösterir.")
    ...
    date_pattern = f"C{future_option}"
    ...
    save_daily_data(matched_rows, 'EOD/BULTEN/warrant_list_call.csv')
    ...
    if not os.path.exists(spot_list_file):
        pd.DataFrame(columns=['DAYANAK VARLIK']).to_csv(spot_list_file, index=False)
    ...
    save_daily_data(not_found_df, 'EOD/BULTEN/spot_list_call.csv')
    ...
    save_daily_data(warrants_data, 'EOD/BULTEN/highest_strike_price_call.csv')
